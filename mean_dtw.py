...
# -*- coding: utf-8 -*-
"""
Pairwise EDR & LCSS distances (unsupervised) for eigenvalue evolution runs.
...
def main():
    ...
    # 4) Preprocess and global normalization
    pre = [_preprocess(L, EPS_FLOOR, USE_LOG1P) for L, _ in raw]
    pre = _global_normalize(pre, NORMALIZE_MODE)

    # 5) Mean curves on common grid
    means = []
    for (L, t), P in zip(raw, pre):
        m = _mean_curve(P)
        means.append(_resample_curve(m, t, tg))

    # 6) Epsilon (either absolute or relative scale rule)
    ...
    # 7) Pairwise EDR & LCSS on mean curves
    N = len(means)
    ...
    for i in range(N):
        for j in range(i + 1, N):
            d1 = edr_distance(means[i], means[j], eps=eps, gap_cost=EDR_GAP_COST)
            d2 = lcss_distance(means[i], means[j], eps=eps)
            D_edr[i, j] = D_edr[j, i] = d1
            D_lcss[i, j] = D_lcss[j, i] = d2

    # 8) Save CSVs with header row & col
    ...
    # 9) Optional quick print: if names use 'r' prefix for random, show separation stats
