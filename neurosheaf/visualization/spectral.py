...
class SpectralVisualizer:
    ...
    def plot_eigenvalue_evolution(self,
        ...
        # Limit number of eigenvalues if specified
        ...
        # Create trace for each eigenvalue path
        for i in range(max_k):
            eigenvalue_path = eigen_paths[i, :]
            
            # Skip entirely NaN paths
            if np.all(np.isnan(eigenvalue_path)):
                continue
            ...
            hover_texts = []
            for j, (param, eigenval) in enumerate(zip(filtration_params, eigenvalue_path)):
            ...
            fig.add_trace(go.Scatter(
                ...
                visible=True if i < 20 else 'legendonly'  # Show first 20, rest in legend only
            ))
        
        # Update layout with appropriate y-axis scaling
        y_axis_config = {
        ...
        if use_log_scale:
            ...
            if len(non_nan_values) > 0:
                ...
                y_axis_config['range'] = [np.log10(min_val * 0.1), np.log10(max_val * 10)]
